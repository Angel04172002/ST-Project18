...
export class AbsenceComponent implements OnInit {
    ...
    async getAbsences() {
        ...
        this.students = [];
        if (type === "Teacher") {
            await firstValueFrom(this.http.getAbsencesByTeacher(id)).then(
                (data) => {
                    for (let item of data) {
                        ...
                        let absencesDataArray = {
                            id: item.id,
                            studentId: item.absence_student_id,
                        ...
                        if (
                            this.gradeDivisions.indexOf(
                                item.grade_division_id
                            ) === -1
                        ) {
                        ...
                        if (
                            this.subjects.indexOf(item.absence_subject_id) ===
                            -1
                        ) {
            ...
            await firstValueFrom(this.http.getAbsencesByGradeTeacher(id)).then(
                (data) => {
                    for (let item of data) {
                        for (let i of this.excuseReasons) {
                            if (i.absenceId === item.id) {
            ...
            await firstValueFrom(this.http.getAbsencesByStudent(id)).then(
                (data) => {
                    for (let item of data) {
                        for (let i of this.excuseReasons) {
                        ...
                        let absencesDataArray = {
                            ...
                            teacherCreator: item.teacher_creator_id
                                ? item.teacher_creator_id
                                : "",
                            gradeTeacherCreator: item.grade_teacher_creator_id
                                ? item.grade_teacher_creator_id
                                : "",
                            absenceType: item.absence_type_id,
                            absenceReason: excuseReason,
                        ...
                        this.absencesDataForStudents.data.push(
                            absencesDataArray
                        );
            ...
            await firstValueFrom(this.http.getAbsencesByParent(id)).then(
                (data) => {
                    for (let item of data) {
                        for (let i of this.excuseReasons) {
                            if (i.absenceId === item.absence_id) {
        ...
        this.absencesData.data.length != 0
            ? (this.dataSource.data = this.absencesData.data)
            : (this.dataSource.data = this.absencesDataForStudents.data);
    }
    ...
    async addAbsences(row: any) {
        ...
        if (this.absenceType) {
            row.absenceType = this.absenceType;
        }
        ...
        type == "Teacher"
            ? (creator = { teacherId: id, gradeTeacherId: null })
            : (creator = { teacherId: null, gradeTeacherId: id });
    ...
    async updateAbsences(row: any) {
        ...
        let req = this.http.updateAbsencesByTeacher(row.id, absences, creator);
        ...
        if (type === "Parent") {
            try {
                await firstValueFrom(
                    this.http.getExcuseReasonsByParent(id)
                ).then((data) => {
                    for (let item of data) {
            ...
            try {
                await firstValueFrom(
                    this.http.getExcuseReasonsByStudent(id)
                ).then((data) => {
                    for (let item of data) {
                        let excuseReasonsArray = {
                            absenceId: item.id,
            ...
            try {
                await firstValueFrom(
                    this.http.getExcuseReasonsByTeacher(id)
                ).then((data) => {
                    console.log(data);
    ...
    async getStudents(grade: Grade["id"], division: GradeDivision["id"]) {
        ...
        if (type === "Teacher") {
            try {
                await firstValueFrom(
                    this.http.getStudentsByGradeAndDivision(grade, division)
                ).then((data) => {
    ...
    getStudentIdSwitch(row: any) {
        for (let i in this.students) {
            if (
                row.firstName === this.students[i].firstName &&
                row.lastName === this.students[i].lastName
            ) {
    ...
    async getGradeDivisions() {
        ...
        if (type === "Teacher") {
            try {
                await firstValueFrom(
                    this.http.getGradesDivisionsAndSubjectsForTeacher(id)
                ).then((data) => {
                    data = data[0];

                    if (
                        this.gradeDivisions.indexOf(
                            data.teacher_grade_division_id
                        ) === -1
                    ) {
                        this.gradeDivisions.push(
                            data.teacher_grade_division_id
                        );
                    }
            ...
            try {
                await firstValueFrom(
                    this.http.getGradesDivisionsAndSubjectsForGradeTeacher(id)
                ).then((data) => {
                    data = data[0];
            ...
            this.dataSource.data = this.absencesData.data.filter((item) => {
                return (
                    (this.gradeSelect === undefined ||
                        item.grade === this.gradeSelect) &&
                    (this.gradeDivisionSelect === undefined ||
                        item.gradeDivision === this.gradeDivisionSelect) &&
                    (this.yearTermsSelect === undefined ||
                        item.term === this.yearTermsSelect)
                );
            });
        } else if (type === "Student" || type === "Parent") {
            this.dataSource.data = this.absencesDataForStudents.data.filter(
                (item) => {
                    return (
                        (this.yearTermsSelect === undefined ||
                            item.term === this.yearTermsSelect) &&
                        (this.subjectSelect === undefined ||
                            item.absenceSubject === this.subjectSelect)
                    );
                }
            );
        }
    ...
    showChecked(event: MatCheckboxChange): void {
        if (event.checked === true) {
            this.absenceType = AbsenceTypes.Excused;
        } else {
            this.absenceType = AbsenceTypes.Unexcused;
        }
    ...
    async addExcuseReasons() {
        ...
        let binary = atob(this.fileAsBinaryString);
        let array = [];
        for (let i = 0; i < binary.length; i++) {
            array.push(binary.charCodeAt(i));
        }
        let fileBlob = new Blob([new Uint8Array(array)], {
            type: "image/jpeg",
        }); // Adjust the MIME type as necessary

        // Create FormData and append the file
        let formData = new FormData();
        formData.append("file", fileBlob, "filename.jpg"); // Provide a filename
        formData.append("excuseReasons", JSON.stringify(excuseReasons)); // Append other data as JSON

        let req = this.http.addExcuseReasonsByParent(formData);
